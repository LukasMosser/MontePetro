...
class Region(object):
	...
	def AddPropertiesFromParameterClass(self):
		...
		self.properties = dict([(param, Property(param_cls[param],self.model.GlobalRandGen.GetRandomInteger(10000,1000000,1),self.model.N)) for param in parcls_dict(self.parameter_class)])
	
	def AddLayerProperties(self,layer_prop_dict):
		for prop_key in layer_prop_dict:
			self.properties[prop_key] = Property(layer_prop_dict[prop_key],self.model.GlobalRandGen.GetRandomInteger(10000,1000000,1),self.model.N)
	
	def AddProperty(self,param,args):
		self.properties[param] = Property(args,self.model.GlobalRandGen.GetRandomInteger(10000,1000000,1),self.model.N)
	...
	def PrintProperties(self):
		...
		print "Properties:"
		for property in self.properties:
			print "\tName: ", self.properties[property].name,"\n\tProbability Distribution Function: ", self.properties[property].distribution, "\n\tMean of Samples: ", self.properties[property].mean,  "\n\tMode of Samples: ", self.properties[property].mode
	...
	def Dump_Region(self):
		for key in self.properties:
			numpy.savetxt(""+str(self.region_name)+"_"+str(self.properties[key].name)+".csv",self.properties[key].samples,delimiter=",")
		for key in self.calculated_properties:
			numpy.savetxt(""+str(self.region_name)+"_"+str(self.calculated_properties[key].name)+".csv",self.calculated_properties[key].values,delimiter=",")
